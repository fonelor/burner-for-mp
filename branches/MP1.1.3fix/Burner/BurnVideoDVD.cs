...
namespace Burner
{
  public class BurnVideoDVD
  {
                               ...
                               string PathtoDVDBurnExe, string RecorderDrive)
    {
      ...
      // Make the DVD dir. Gets deleted straight away, but saves an exception  FIXME
    ...
    private void MenuGeneration()
    {
      /*try
      {
        ...
        using (StreamWriter SW_MenuGen = File.CreateText(Path.Combine(_TempFolderPath, "menuGen.gen")))
        {
          ...
          SW_MenuGen.WriteLine(@"Graphics Magick =" + Config.GetFile(Config.Dir.Base, @"Burner\", "gm.exe"));
          SW_MenuGen.WriteLine(@"Mplex =" + Config.GetFile(Config.Dir.Base, @"Burner\", "mplex.exe"));
          SW_MenuGen.WriteLine(@"jpeg2yuv =" + Config.GetFile(Config.Dir.Base, @"Burner\", "png2yuv.exe"));
          SW_MenuGen.WriteLine(@"mpeg2enc =" + Config.GetFile(Config.Dir.Base, @"Burner\", "mpeg2enc.exe"));
          SW_MenuGen.WriteLine(@"spumux =" + Config.GetFile(Config.Dir.Base, @"Burner\", "spumux.exe"));
          SW_MenuGen.WriteLine(@"AC3 audio =" + Config.GetFile(Config.Dir.Base, @"Burner\", "Silence.ac3"));
          SW_MenuGen.WriteLine(@"Button Image =" + Config.GetFile(Config.Dir.Base, @"Burner\", "navButton.png"));
          ...
          for (int i = 0; i < NumberOfFiles; i++)
          {
            ...
            strTemp = Path.GetFileName((string)_FileNames[i]);// There was __FilesToBurn[i]
        ...
        string SourceFile = Config.GetFile(Config.Dir.Base, @"Burner\", "menuGen.exe");
    ...
    private void ConfigXMLCreation()
    {
      try
      {
        ...
        using (StreamWriter SW_ConfigFile = File.CreateText(Path.Combine(_TempFolderPath, "Config.xml")))
        {
          ...
          for (int i = 0; i < NumberOfFiles; i++)
          {
            string smBkgd = "menuSubBackground." + i.ToString() + ".menu.mpg";
            ...
            SW_ConfigFile.WriteLine("    <video format=\"" + _TvFormat + "\" />");
            SW_ConfigFile.WriteLine("      <pgc>");
            strTemp = "        <vob file=\"" + Path.Combine(_TempFolderPath, "F") + (i + 1).ToString() + ".mpg" +
                      "\" chapters=\"15:00,30:00,45:00,1:00:00,1:15:00,1:30:00,1:45:00,2:00:00,2:15:00,2:30:00,2:45:00,3:00:00\" />";// FIXME There was _FilesToBurn[i]
            SW_ConfigFile.WriteLine(strTemp);
    ...
    private void DVDFilesCreation()
    {
      try
      {
        ...
        BurnerProcess.StartInfo.FileName = Config.GetFile(Config.Dir.Base, @"Burner\", "dvdauthor.exe");
    ...
    private void ISOFileCreation()
    {
      ...
      try
      {
        ...
        BurnerProcess = new Process();
        BurnerProcess.EnableRaisingEvents = true;
        BurnerProcess.StartInfo.WorkingDirectory = Config.GetSubFolder(Config.Dir.Base, @"Burner\");
        BurnerProcess.StartInfo.UseShellExecute = false;
        ...
        BurnerProcess.StartInfo.FileName = Config.GetFile(Config.Dir.Base, @"Burner\", "mkisofs.exe");
    ...
    private void WriteDVD()
    {
      try
      {
        _CurrentProcess = "Burning the DVD - wodim.exe";
        ...
        if (_BurnTheDVD == true)
        {
          ...
          BurnerProcess.StartInfo.FileName = Config.GetFile(Config.Dir.Base, @"Burner\", "wodim.exe");

          string args = "-dev='" + _RecorderDrive + "' " + Path.Combine(_TempFolderPath, "dvd.iso");
          
          BurnerProcess.StartInfo.Arguments = args;

          BurnerProcess.Exited += new EventHandler(BurnProcess_Exited);
          
          Log.Info("Starting wodim", "Args: " + args);
          BurnerProcess.Start();
        ...
        case ConvertState.VideoConversion:
          {
            ...
            string DestinationFilePath = _TempFolderPath;
            ...
            try
            {
              ...
              BurnerProcess.StartInfo.FileName = Config.GetFile(Config.Dir.Base, @"Burner\", "mencoder.exe");
